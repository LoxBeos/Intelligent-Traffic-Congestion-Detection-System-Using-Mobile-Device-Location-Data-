...
let historicalData = [];
let userReports = [];
let currentJamCount = 0; // Hàm cập nhật cấu hình kẹt xe
...
async function classifyTrafficJam(locations) {
  ...
  const speedThresholdAdjustment = isRushHour ? 0.8 : 1;
  const deviceCountThresholdAdjustment = isRushHour ? 0.8 : 1;
  ...
  const trafficJams = await Promise.all(groups.filter(group => {
      if (group.length < trafficJamConfig.lightJam.minDevices) return false; // Không đủ thiết bị
      ...
      // Kiểm tra tốc độ trung bình và thời gian đứng yên của các thiết bị
  ...
  // Xử lý các báo cáo từ người dùng nếu không có thiết bị nào trong khu vực
  const additionalTrafficJams = userReports.filter(report => {
      ...
      // Nếu chỉ có báo cáo từ người dùng, coi đó là kẹt xe nhẹ
      return {
...
}, 60000); // Kiểm tra tình trạng giao thông lại mỗi phút
